    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent
        ...
        /* UI: comfortable on desktop, and on very small screens it shrinks to
           ...
           `fx-panel` carries the light-mode (candy/pink) contrast rules, see
           src/styles/buttons.css → EXPERIENCE PANEL. */
        className="fx-panel w-[min(340px,calc(100vw-1rem))] overflow-hidden rounded-2xl border-border/60 bg-background/95 p-2 shadow-2xl backdrop-blur-xl sm:w-[320px]"
      >
        <p className="fx-title mb-1.5 px-3 pt-2 text-[9px] uppercase tracking-[0.22em] text-muted-foreground">
          Experience
        </p>
          ...
          {rows.map(({ id, label, hint, icon: Icon, value, set }) => (
            <li key={id}>
              <label
                ...
                className="fx-row grid min-h-[56px] cursor-pointer grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-x-3 gap-y-1 rounded-xl px-3 py-3 transition-colors hover:bg-foreground/5 active:bg-foreground/10 sm:min-h-0 sm:py-2.5"
              >
                <Icon className="fx-icon h-4 w-4 shrink-0 text-muted-foreground" aria-hidden="true" />
                <span className="min-w-0">
                  <span className="fx-label block text-[13px] font-medium leading-tight">{label}</span>
                  {/* Hints wrap instead of truncating, so no text is cut off. */}
                  <span className="fx-hint mt-0.5 block break-words text-[10px] leading-snug text-muted-foreground">
                    {hint}
                  </span>
                </span>
        ...
        <div className="fx-divider mt-2 space-y-1.5 border-t border-border/50 px-3 pb-1 pt-3">
          {prefersReducedMotion && (
            <p className="fx-note text-[10px] leading-snug text-muted-foreground">
