...
-- ---------- 8. portfolio_clicks (outbound click analytics) ------------------
-- Tracks every open of a portfolio Website tile / preview / "Visit site" CTA.
--   kind = 'tile'     → tile clicked (opened the preview modal)
--   kind = 'preview'  → preview modal was opened (same event, logged from modal)
--   kind = 'visit'    → user clicked "Visit site" in modal or a detail page
...
CREATE INDEX IF NOT EXISTS portfolio_clicks_created_idx ON public.portfolio_clicks (created_at DESC);
CREATE INDEX IF NOT EXISTS portfolio_clicks_slug_idx    ON public.portfolio_clicks (slug);
GRANT INSERT ON public.portfolio_clicks TO anon, authenticated;
GRANT ALL    ON public.portfolio_clicks TO service_role;
ALTER TABLE public.portfolio_clicks ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "portfolio_clicks public insert" ON public.portfolio_clicks;
CREATE POLICY "portfolio_clicks public insert" ON public.portfolio_clicks
  FOR INSERT TO anon, authenticated WITH CHECK (true);
-- Reads are server-only via supabaseAdmin from getPortfolioClickStats().
